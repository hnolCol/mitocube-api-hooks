...
export function createOpenAIAPI(client) {
 ...
 */
async function getCypherQuery_API({ prompt, session_id }) {
    const res = await client.get(`/ai/openai/cypher`, {params: { prompt, session_id }})
    return res.data
}

const useGetCypherQuery = (APIParams = { prompt, session_id }, useQueryOptions = { staleTime: 5000 }) => {
    return useQuery({
 ...
 * @returns {import("./types").OpenAIResponse} Cypher query response that interprets the data.
 */
async function getPubmedPublicationSummary_API({ tag, limit = 5, sort_by = "relevance" }) {
    const res = await client.get(`/ai/openai/literature/feature/proteins/${tag}`, {params: { tag, limit, sort_by }})
    return res.data
}

const useGetPublicationSummaryForProtein = (APIParams = { tag, limit, sort_by }, useQueryOptions = { staleTime: 5000 }) => {
    return useQuery({
